...
document.addEventListener("DOMContentLoaded", async () => {
  ...
  if (profileData) {
    ...
    if (welcomeTitleEl && profileData) {
      welcomeTitleEl.textContent = `Bem-vindo(a), ${profileData.nome_empresa || profileData.nome}!`;
    }
    if (profileData.logo) {
  ...
  // --- Cache de endereços para performance ---
  const enderecoCache = {};
  async function getEndereco(id) {
    if (enderecoCache[id]) return enderecoCache[id];
    try {
    ...
    if (selectedDayEvents.length > 0) {
      for (const event of selectedDayEvents) {
        ...
        if (event.id_endereco) {
          const e = await getEndereco(event.id_endereco);
          if (e) enderecoText = `${e.bairro}, ${e.cidade}/${e.estado}`;
        }
        ...
        let tipo = event.tipo === "visita_tecnica" ? "Visita Técnica" : event.tipo === "palestra" ? "Palestra" : "Outro";
  ...
  function renderUpcomingEvents() {
    ...
    sortedEvents.forEach(event => {
      ...
      eventDetails.textContent = `${event.tipo === "visita_tecnica" ? "Visita Técnica" : event.tipo} • ${event.localizacao || ''}`;
  ...
  submitRatingBtn.addEventListener("click", () => {
    alert(`Evento "${currentEvent.nome}" avaliado com ${currentRating} estrelas!`);
