...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  try {
    ...
    if (profileData) {
      const nameEl = document.querySelector('#user-profile-btn p');
      if (nameEl) nameEl.textContent = profileData.nome_empresa || profileData.nome || '';
      const avatar = document.querySelector('#user-profile-btn .user-avatar');
      if (avatar && profileData.logo) {
  ...
  try {
    ...
    events = (Array.isArray(raw) ? raw : [])
      .filter(e =>
        e &&
        (['visita_tecnica', 'palestra'].includes(e.tipo)) &&
        ((e.status || '').trim().toLowerCase() === 'aprovada')
      )
      .map(e => ({
        ...e,
